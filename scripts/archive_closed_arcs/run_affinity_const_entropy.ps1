...
$log  = "local-training/affinity_const_entropy.log"
...
# restore clean 43.2% Affinity base (same warm-start as the degrading salvage baseline)
$bak = "E:\mage-trainingackupsffinity_pre_dominance_20260623\model.pt"
Copy-Item $bak "$md\model.pt" -Force; Copy-Item $bak "$md\model_latest.pt" -Force
...
$env:ENTROPY_START="0.10"; $env:ENTROPY_END="0.10"  # CONSTANT (paper recipe)
...
"=== AFFINITY CONST-ENTROPY (warm-start, weighted diet, CP7 skill1) $(Get-Date) ===" | Out-File $log
...
"=== AFFINITY CONST-ENTROPY DONE $(Get-Date) ===" | Out-File $log -Append
