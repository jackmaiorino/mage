# FRESH-TRAIN the wide net (d_model=128/2L) on the FAITHFUL LEAGUE DIET --
...
$out  = "local-training/fresh128_RESULT.log"
$tlog = "local-training/fresh128.log"; $telog = "local-training/fresh128.err"
...
$ck   = "E:\mage-training\backups\fresh128_control"
...
"=== FRESH-128 CONTROL (faithful diet, META-H clock) started $(Get-Date) ; d_model=128/2L ===" | Out-File $out
...
for ($c = 1; $c -le $nChunks; $c++) {
  ...
  if (-not $dimChecked -and (Test-Path "$md\model_latest.pt")) {
    ...
    "DIM CHECK: d_model=$dim (expect 128)" | Out-File $out -Append; $dimChecked = $true
  }
  if ($evalChunks -contains $c) {
    Eval-Now "f128_c$c" 5151; Metric "f128_c$c"; RepSep "f128_c$c"
    Copy-Item "$md\model_latest.pt" "$ck\model_after_c$c.pt" -Force
    $csv = "$GL/f128_c$c/matchups.csv"
    ...
    Clean-Sweep "f128_c$c"
  }
}
"=== FRESH-128 CONTROL DONE $(Get-Date) ; best_wr=$bestWr ; ckpts in $ck ===" | Out-File $out -Append
