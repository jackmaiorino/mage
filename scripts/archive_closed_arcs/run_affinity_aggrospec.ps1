...
$log  = "local-training/affinity_aggrospec.log"
...
$env:DECK_LIST_FILE="$deckDir/decklist.affinity_aggro_specialist.txt"   # weighted opponent pool
...
$env:TOTAL_EPISODES="12000"                             # chunk 1
$env:GAME_LOG_FREQUENCY="2000"
$env:RL_HEURISTIC_STEP_REWARDS="0"                      # terminal-only reward (thesis)

"=== AFFINITY AGGRO-SPEC (warm-start, weighted diet, CP7 skill1) $(Get-Date) ===" | Out-File $log
...
"=== AFFINITY AGGRO-SPEC DONE $(Get-Date) ===" | Out-File $log -Append
