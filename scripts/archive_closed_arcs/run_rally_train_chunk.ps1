# RALLY DOMINANCE TRAINING (Codex #26): warm-start fine-tune on the weighted soft-matchup diet,
...
$log  = "local-training/rally_train_chunk.log"
...
$env:MODEL_PROFILE="Pauper-Rally-Anchor-Value"          # warm-start from existing model.pt
$env:RL_AGENT_DECK_LIST="$deckDir/Deck - Mono Red Rally.dek"
$env:DECK_LIST_FILE="$deckDir/decklist.rally_train_diet.txt"   # weighted opponent pool
...
$env:ACTOR_LR="7e-5"                                    # low fine-tune LR (Codex)
$env:ENTROPY_START="0.10"; $env:ENTROPY_END="0.10"
$env:NUM_GAME_RUNNERS="48"
$env:TOTAL_EPISODES="30000"                             # chunk 1
$env:GAME_LOG_FREQUENCY="2000"
$env:RL_HEURISTIC_STEP_REWARDS="0"                      # terminal-only reward (thesis)

"=== RALLY TRAIN CHUNK1 (warm-start, weighted diet, CP7 skill1) $(Get-Date) ===" | Out-File $log
...
"=== RALLY TRAIN CHUNK1 DONE $(Get-Date) ===" | Out-File $log -Append
