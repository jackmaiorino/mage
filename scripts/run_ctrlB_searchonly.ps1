# CONTROL B: SEARCH_OP generation machinery ON exactly as candq v5 (spy-filtered, 1 activation,
# 2 playouts, 10s/30s timeouts, override OFF) but the learner FULLY INERT (COEF=0,
...
"=== CONTROL B (search-on, learner-inert) started $(Get-Date) ; ${nChunks}x${chunkMin}min ; metric=NO_BOARD ===" | Out-File $out
...
Eval-Now "ctrlB_c0" 5151; Metric "ctrlB_c0"
...
$trainEnv = {
  ...
  $env:CANDIDATE_Q_FROM_MCTS_TARGETS="0"; $env:CANDIDATE_Q_LOSS_COEF="0.0"
  $env:CANDIDATE_Q_MCTS_SIGNED_TARGETS="1"   # keeps the unsigned KL distill path gated off
...
Copy-Item "$bak\model_latest.pt" "$md\model_latest.pt" -Force; Copy-Item "$bak\model.pt" "$md\model.pt" -Force
"=== restored Spy baseline after control run ===" | Out-File $out -Append
"=== CONTROL B DONE $(Get-Date) ===" | Out-File $out -Append
