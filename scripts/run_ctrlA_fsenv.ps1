# CONTROL A: exact fs_* environment (held 0.52 x 8 chunks on Jun 8 code) on CURRENT code.
...
"=== CONTROL A (fs_* env on current code) started $(Get-Date) ; ${nChunks}x${chunkMin}min ===" | Out-File $out
...
Eval-Now "ctrlA_c0" 5151; Metric "ctrlA_c0"
...
for ($c = 1; $c -le $nChunks; $c++) {
  ...
  Eval-Now "ctrlA_c$c" 5151; Metric "ctrlA_c$c"
}
...
"=== CONTROL A DONE $(Get-Date) ===" | Out-File $out -Append
