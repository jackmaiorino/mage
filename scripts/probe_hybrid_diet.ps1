# PROBE (cheap, ~7min): does OPPONENT_SAMPLER=hybrid in the CURRENT code actually engage
...
$env:PBT_EXPLOIT_INTERVAL_MINUTES="999"   # no PBT churn during probe
...
Write-Output "=== opponent_type distribution of NEW rows (hybrid diet) ==="
if ($after -gt $before) {
...
} else { Write-Output "NO NEW ROWS -- trainer failed to produce episodes; check $telog" }
Write-Output "=== trainer.err tail ==="
Get-Content $telog -Tail 8 -ErrorAction SilentlyContinue