...
$ck   = "local-training\backups\bignet_league_20260616"
...
"=== BIGNET LEAGUE (faithful diet, META-H clock) started $(Get-Date) ; d_model=256/4L ===" | Out-File $out
...
# Promote ONLY Spy; keep meta profiles UNQUALIFIED (promoted=false, baseline_wr=0)
# so the league serves them as META-H (CP7 on their pool decks) -- no 128 NN load.
py -3.12 -c "
import json,os
base='Mage.Server.Plugins/Mage.Player.AIRL/src/mage/player/ai/rl/profiles'
for p,promote,wr in [('Pauper-Spy-Combo-Value',True,0.45),('Pauper-Wildfire-Value',False,0.0),('Pauper-Rally-Anchor-Value',False,0.0),('Pauper-Affinity-Anchor-Value',False,0.0)]:
    f=f'{base}/{p}/logs/league/agent_status.json'
    os.makedirs(os.path.dirname(f),exist_ok=True)
    d=json.load(open(f)) if os.path.exists(f) else {}
    d['promoted']=promote; d['baseline_wr']=wr
    json.dump(d,open(f,'w'),indent=2)
print('Spy promoted; meta profiles UNQUALIFIED -> META-H clock')
...
for ($c = 1; $c -le $nChunks; $c++) {
  ...
  if ($evalChunks -contains $c) {
    Eval-Now "bl_c$c" 5151; Metric "bl_c$c"; RepSep "bl_c$c"
    Copy-Item "$md\model_latest.pt" "$ck\model_after_c$c.pt" -Force
    $csv = "$GL/bl_c$c/matchups.csv"
...
"=== BIGNET LEAGUE DONE $(Get-Date) ; best_wr=$bestWr ; ckpts in $ck ===" | Out-File $out -Append
