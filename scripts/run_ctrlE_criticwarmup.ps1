# CONTROL E: exact fs_* environment (held 0.52 x 8 chunks on Jun 8 code) on CURRENT code.
...
# If ctrlE holds ~0.52 -> code clean, collapse is search-linked -> run Control B.
# If ctrlE collapses -> code regression in 16b153d906 -> bisect stride/sentinels.
Set-Location "C:\Users\Jack\IdeaProjects\mage"
$ErrorActionPreference = "Continue"
$out  = "local-training/ctrlE_RESULT.log"
$tlog = "local-training/ctrlE_train.log"; $telog = "local-training/ctrlE_train.err"
...
"=== CONTROL E (fs_* env on current code) started $(Get-Date) ; ${nChunks}x${chunkMin}min ===" | Out-File $out
...
Eval-Now "ctrlE_c0" 5151; Metric "ctrlE_c0"
...
$trainEnv = {
  ...
  $env:OPPONENT_SAMPLER="league"; $env:LEAGUE_PROMOTE_WR="0.40"; $env:LEAGUE_POST_HEURISTIC_SKILL="1"; $env:LEAGUE_MODE=""
  $env:CRITIC_WARMUP_STEPS="132900"; $env:FREEZE_ENCODER_IN_WARMUP="1"; $env:LOSS_SCHEDULE_ENABLE="1"
...
for ($c = 1; $c -le $nChunks; $c++) {
  ...
  Eval-Now "ctrlE_c$c" 5151; Metric "ctrlE_c$c"
}
...
"=== CONTROL E DONE $(Get-Date) ===" | Out-File $out -Append
