...
$out  = "local-training/wildfire_cleanrun_eval_RESULT.log"
$baseReg = "local-training/_brew_win_registry.json"
$reg  = "local-training/_wildfire_gauntlet_registry.json"
$gauntlet = "Mage.Server.Plugins/Mage.Player.AIRL/src/mage/player/ai/decks/Pauper/decklist.all_opponents.txt"
$md   = "Mage.Server.Plugins\Mage.Player.AIRL\src\mage\player\ai\rl\profiles\Pauper-Wildfire-Value\models"
...
"=== WILDFIRE GAUNTLET BASELINE $(Get-Date) ===" | Out-File $out
...
for e in r:
    if e.get('profile')=='Pauper-Wildfire-Value': e['deck_path']=r'$gauntlet'
...
py -3.12 scripts/run_cp7_eval_sweep.py --registry $reg --profiles Pauper-Wildfire-Value `
  --opponents "grixis,burn,faeries,terror,wildfire,caw,elves,rally" --skill 1 --games-per-matchup 64 --parallel 8 --games-per-job 16 `
  --eval-game-logging --skip-compile --replay-seed-base 5151 --run-id "wildfire_cleanrun_eval" 2>&1 |
  Select-String "wr=" | ForEach-Object { $_.Line } | Out-File $out -Append
"=== WILDFIRE GAUNTLET BASELINE DONE $(Get-Date) ===" | Out-File $out -Append
