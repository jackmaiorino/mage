# CONTROL G: exact fs_* environment (held 0.52 x 8 chunks on Jun 8 code) on CURRENT code.
...
$chunkMin = [int]($env:CHUNK_MIN); if ($chunkMin -le 0) { $chunkMin = 60 }
...
function Eval-Now($rid, $seed) {
  ...
  Remove-Item Env:\PY_SERVICE_MODE -ErrorAction SilentlyContinue   # eval must match prior anchors (hybrid default)
...
"=== CONTROL G (CP7-1 diet + PURE-TORCH inference, no ONNX in loop) started $(Get-Date) ; ${nChunks}x${chunkMin}min ===" | Out-File $out
...
$trainEnv = {
  ...
  $env:INFER_CUDA_DEVICE="cuda:1"; $env:TRAIN_CUDA_DEVICE="cuda:0"
  $env:PY_SERVICE_MODE="shared_gpu"; $env:TRAIN_PROFILES="1"; $env:NUM_GAME_RUNNERS="64"; $env:TOTAL_EPISODES="99999999"
}
for ($c = 1; $c -le $nChunks; $c++) {
  ...
  Eval-Now "ctrlG_c$c" 5151; Metric "ctrlG_c$c"
}
...
"=== CONTROL G DONE $(Get-Date) ===" | Out-File $out -Append
