...
$log  = "local-training/affinity_train_chunk.log"
...
$env:DECK_LIST_FILE="$deckDir/decklist.affinity_train_diet.txt"   # weighted opponent pool
...
$env:TOTAL_EPISODES="40000"                             # chunk 1
$env:GAME_LOG_FREQUENCY="2000"
$env:RL_HEURISTIC_STEP_REWARDS="0"                      # terminal-only reward (thesis)

"=== AFFINITY TRAIN CHUNK1 (warm-start, weighted diet, CP7 skill1) $(Get-Date) ===" | Out-File $log
...
"=== AFFINITY TRAIN CHUNK1 DONE $(Get-Date) ===" | Out-File $log -Append
