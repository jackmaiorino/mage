# CONTROL F: exact fs_* environment (held 0.52 x 8 chunks on Jun 8 code) on CURRENT code.
...
# If ctrlF holds ~0.52 -> code clean, collapse is search-linked -> run Control B.
# If ctrlF collapses -> code regression in 16b153d906 -> bisect stride/sentinels.
Set-Location "C:\Users\Jack\IdeaProjects\mage"
$ErrorActionPreference = "Continue"
$out  = "local-training/ctrlF_RESULT.log"
$tlog = "local-training/ctrlF_train.log"; $telog = "local-training/ctrlF_train.err"
...
"=== CONTROL F (pure CP7-skill-1 eval-matched diet) started $(Get-Date) ; ${nChunks}x${chunkMin}min ===" | Out-File $out
...
for ($c = 1; $c -le $nChunks; $c++) {
  ...
  Eval-Now "ctrlF_c$c" 5151; Metric "ctrlF_c$c"
}
...
"=== CONTROL F DONE $(Get-Date) ===" | Out-File $out -Append
