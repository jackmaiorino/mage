# WORLD-MODEL AUX + KL ANCHOR on the 128 (the post-label-noise-reframe lever).
# candidate_q is closed (label noise, not capacity). Instead de-myopia the value
# head with a LOW-VARIANCE, LEARNABLE self-supervised target: predict own future
# zone-counts (6 features x horizons {8,24,64}) from the shared CLS. Pair with a
# reference-policy KL anchor (protects execution from shared-encoder collateral --
...
$ck   = "E:\mage-training\backups\worldmodel_aux_20260617"
$ref  = "E:\mage-training\backups\ref_fresh128.pt"
$teacher = "E:\mage-training\backups\fresh128_control\model_best.pt"
...
function Clean-Sweep($rid) {
  # keep game_logs (needed for value-AUC) but drop the heavy snapshot/db dirs
...
"=== WORLD-MODEL AUX + KL ANCHOR (128, faithful diet) started $(Get-Date) ; WM_COEF=$($env:WM_COEF) KL=$($env:KL_COEF) ===" | Out-File $out
Kill-Train; Start-Sleep -Seconds 4
# continue from the competent fresh-128 (NOT fresh-init); freeze a reference copy for the KL anchor
if (-not (Test-Path $teacher)) { "FATAL: fresh-128 teacher missing at $teacher" | Out-File $out -Append; exit 1 }
...
print('continue-from fresh-128; all promoted -> META-RL clock + mirror + CP7')
" 2>&1 | Out-File $out -Append
# baseline eval of the teacher (so winrate deltas are measured, not absolute)
Eval-Now "wm_c0" 5151; Metric "wm_c0"; Clean-Sweep "wm_c0"
...
for ($c = 1; $c -le $nChunks; $c++) {
  ...
  # THE LEVER: world-model aux (de-myopia value) + reference-policy KL anchor (protect execution)
  # WORLD_MODEL_DIAG=1 -> prints "[WORLD_MODEL_DIAG] valid=N/rows dim= coef= loss=" so we can
  # CONFIRM the aux is firing (nonzero valid rows + nonzero loss), not silently no-op'ing (Codex #4).
  ...
  $env:ENTROPY_START="0.10"; $env:ENTROPY_END="0.03"; $env:ENTROPY_DECAY_STEPS="60000"  # lower start: continuing from competent, less exploration
  ...
  "=== chunk $c train start $(Get-Date) (WM=$wmCoef KL=$klCoef) ===" | Out-File $out -Append
  ...
  if ($evalChunks -contains $c) {
    Eval-Now "wm_c$c" 5151; $m = Metric "wm_c$c"
    Copy-Item "$md\model_latest.pt" "$ck\model_after_c$c.pt" -Force
    # FUNNEL-COLLAPSE GATE (Codex #4): abort EARLY on the May collateral-damage signature
    # (cast_spy / finisher_avail collapse) even if winrate has not yet crossed 0.35.
    # Baseline wm_c0: cast_spy=60% finisher_avail=87% NO_BOARD=13%.
    ...
    if ($castSpy -lt 45 -or $finAvail -lt 65) {
      "=== ABORT (funnel collapse): cast_spy=$castSpy% finisher_avail=$finAvail% (baseline 60/87) -> shared-encoder collateral. best=$bestWr preserved. ===" | Out-File $out -Append; break
    }
    $csv = "$GL/wm_c$c/matchups.csv"
    if (Test-Path $csv) { $row = Import-Csv $csv | Select-Object -First 1; $wr=[double]$row.wins/[double]$row.total
      ...
      if ($lowStreak -ge 2) { "=== ABORT: 2 evals <0.35 (anchor too weak / aux too strong). best=$bestWr preserved. ===" | Out-File $out -Append; break }
    }
    Clean-Sweep "wm_c$c"
  }
}
"=== WORLD-MODEL AUX DONE $(Get-Date) ; best_wr=$bestWr ; ckpts $ck ===" | Out-File $out -Append
