...
$out = "local-training/rally_baseline_det.log"
...
$arms = @(
  @{ label="postc1"; src="E:\mage-training\backups\rally_dominant_c1_20260623\model.pt" },
...
foreach($a in $arms){
  ...
  py -3.12 scripts/run_cp7_eval_sweep.py --registry local-training/_rally_gauntlet_registry.json --profiles Pauper-Rally-Anchor-Value `
    --skill 1 --games-per-matchup 256 --games-per-job 16 `
    --deterministic-eval --skip-compile --replay-seed-base 5151 --run-id "rally_det_$($a.label)" 2>&1 |
