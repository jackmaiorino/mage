...
$reg  = "local-training/_brew_cons_registry.json"
$deck = "Mage.Server.Plugins/Mage.Player.AIRL/src/mage/player/ai/decks/Pauper/Deck - Spy Consistency.dek"
...
$ck   = "E:\mage-training\backups\brew_consistency_20260619"
...
"=== BREW CONSISTENCY (fresh-128, faithful diet, consistency deck) started $(Get-Date) ===" | Out-File $out
...
for ($c = 1; $c -le $nChunks; $c++) {
  ...
  if ($evalChunks -contains $c) {
    Eval-Now "bc_c$c" 5151; Metric "bc_c$c"; ReachMetric "bc_c$c"
...
"=== BREW CONSISTENCY DONE $(Get-Date) ; best_wr=$bestWr ; ckpts $ck ===" | Out-File $out -Append
