# CONTROL D: exact fs_* environment (held 0.52 x 8 chunks on Jun 8 code) on CURRENT code.
...
# If ctrlD holds ~0.52 -> code clean, collapse is search-linked -> run Control B.
# If ctrlD collapses -> code regression in 16b153d906 -> bisect stride/sentinels.
Set-Location "C:\Users\Jack\IdeaProjects\mage"
$ErrorActionPreference = "Continue"
$out  = "local-training/ctrlD_RESULT.log"
$tlog = "local-training/ctrlD_train.log"; $telog = "local-training/ctrlD_train.err"
...
"=== CONTROL D (fs_* env on current code) started $(Get-Date) ; ${nChunks}x${chunkMin}min ===" | Out-File $out
...
Eval-Now "ctrlD_c0" 5151; Metric "ctrlD_c0"
...
for ($c = 1; $c -le $nChunks; $c++) {
  ...
  Eval-Now "ctrlD_c$c" 5151; Metric "ctrlD_c$c"
}
...
"=== CONTROL D DONE $(Get-Date) ===" | Out-File $out -Append
