...
$env:MODEL_PROFILE="Pauper-Wildfire-Value"          # warm-start from existing model.pt
$env:RL_AGENT_DECK_LIST="$deckDir/Deck - Jund Wildfire.dek"
$env:DECK_LIST_FILE="$deckDir/decklist.wildfire_gauntlet_diet.txt"   # weighted opponent pool
...
$env:ACTOR_LR="2e-4"                                    # low fine-tune LR (Codex)
$env:ENTROPY_START="0.15"; $env:ENTROPY_END="0.02"
...
"=== WILDFIRE CLEAN-RUN (warm-start, weighted diet, CP7 skill1) $(Get-Date) ===" | Out-File $log
...
"=== WILDFIRE CLEAN-RUN DONE $(Get-Date) ===" | Out-File $log -Append
