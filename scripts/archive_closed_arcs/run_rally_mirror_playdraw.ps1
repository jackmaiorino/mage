...
Copy-Item "E:\mage-training\backups\rally_latest_armF_20260630\model.pt" "$md\model.pt" -Force
Copy-Item "E:\mage-training\backups\rally_latest_armF_20260630\model.pt" "$md\model_latest.pt" -Force
...
py -3.12 scripts/run_cp7_eval_sweep.py --registry local-training/_rally_gauntlet_registry.json --profiles Pauper-Rally-Anchor-Value `
  ...
  --deterministic-eval --skip-compile --replay-seed-base 5151 --run-id "mirror_play256" 2>&1 |
...
py -3.12 scripts/run_cp7_eval_sweep.py --registry local-training/_rally_gauntlet_registry.json --profiles Pauper-Rally-Anchor-Value `
  --opponents "rally" --skill 7 --games-per-matchup 256 --games-per-job 16 `
  --deterministic-eval --skip-compile --replay-seed-base 5151 --run-id "mirror_draw256" 2>&1 |
  Select-String "wr=" | ForEach-Object { $_.Line } | Out-File $out -Append
